...
```sql
MariaDB [aanchal_2cse12]> SELECT MAX(SAL) AS MAX_SALARY,
...
MariaDB [aanchal_2cse12]> SELECT ENAME, SAL, COMM
    -> FROM EMPLOYEE
    -> WHERE COMM > (0.25 * SAL);
 ```
**OUTPUT:**
...
``` sql
MariaDB [aanchal_2cse12]> SELECT ENAME,
...
```sql
MariaDB [aanchal_2cse12]> SELECT JOB,
...
```sql
MariaDB [aanchal_2cse12]> SELECT COUNT(*) AS TOTAL_EMPLOYEES,
...
```sql
MariaDB [aanchal_2cse12]> SELECT DATE_SUB(
    ->  LAST_DAY(CURDATE()),
...
```sql
MariaDB [aanchal_2cse12]> SELECT DEPTNO, COUNT(*) AS TOTAL_EMPLOYEES
    -> FROM EMPLOYEE
    -> GROUP BY DEPTNO
    -> ORDER BY DEPTNO;
```
...
``` sql
MMariaDB [aanchal_2cse12]> SELECT JOB, COUNT(*) AS TOTAL_EMPLOYEES
    -> FROM EMPLOYEE
    -> GROUP BY JOB;
```
**OUTPUT:**
...
```sql
MariaDB [aanchal_2cse12]> SELECT DEPTNO, SUM(SAL) AS TOTAL_SALARY
    -> FROM EMPLOYEE
    -> GROUP BY DEPTNO;
```
**OUTPUT:**
